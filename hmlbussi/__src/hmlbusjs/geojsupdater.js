...
function parseRoutes() {
    ...
    geotext= 'var bus_routes = ' + geotext + ";";
    writeTextFile("www/js/bus_routes.js", geotext);
    //console.log(geotext);

    //pysäkit
    const src_stops = fs.readFileSync('rawdata/stops.txt', 'utf-8');
    let pgeotext = "{\n  \"type\": \"FeatureCollection\",\n  \"features\": [\n";
    src_stops.split(/\r?\n/).forEach(rivi => {
        rivi = rivi.replaceAll(", ", "_");
        rivi = rivi.replaceAll("\"", "");
        //console.log("riV", rivi)
        let [stop_id, stop_code, stop_name, stop_lat, stop_lon] = rivi.split(",");
        if (rivi.length > 20 && stop_id != "stop_id") {
    ...
    pgeotext = pgeotext.slice(0, -2);
    pgeotext += '\n   ]\n}\n';
    //writeTextFile("www/geojson/stopshml.geojson", pgeotext);
    pgeotext= 'var bus_stops = ' + pgeotext + ";";
    writeTextFile("www/js/bus_stops.js", pgeotext);
